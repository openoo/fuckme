...
var MAC={
        ...
        'Init':function($obj,$mid,$jumpurl){
            try {
                $($obj).autocomplete(maccms.path + '/index.php/ajax/suggest?mid=' + $mid, {
                    ...
                    width: '100%', scrollHeight: 380, minChars: 1, matchSubset: 1,
    ...
    'History': {
        ...
        'Init':function(){
			...
			$bodymask=$('<div class="page-mask history-bg"><p>点击任意处返回！</p></div>');
			 $(".mac_history_button").on("click", function(e){
				 if(MAC.History.BoxShow==0){
					$('.mac_history_box').slideDown();
					$('body').append($bodymask);
					document.body.style.overflow='hidden';
					document.addEventListener("touchmove",function(e){e.preventDefault()},false);
					//修复首页透明背景变蓝					
					if($("body").scrollTop() < 1){
						$("#header-index-nav").addClass("header-nav-fixed");
						$("#header-index-wrap").css('background-color','rgba(7,165,223, 1)'); 	
					};								
					MAC.History.BoxShow=1;
				 }else{
					$('.mac_history_box').slideUp();
					document.body.style.overflow='';//出现滚动条
					document.removeEventListener("touchmove",function(e){e.preventDefault()},false);				
					//修复首页透明背景变蓝
					if($("body").scrollTop() < 1){
						$("#header-index-nav").removeClass("header-nav-fixed");	
						$("#header-index-wrap").css('background-color','rgba(7, 165, 223, 0)');			
					};	
					$bodymask.remove();						
			 ...
			 $(".mac_history_box").on("click", function(e){
				 e.stopPropagation();
			 });
            ...
            html = '<dl class="mac_history_box" >';
            html +='<dt><span>观看记录</span><a class="iconfont icon-qingkong" target="_self" href="javascript:void(0)" onclick="MAC.History.Clear();"></a></dt>';
            ...
            if(jsondata.length > 0){
                for($i=0; $i<jsondata.length; $i++){
                    ...
                    html +='<a href="'+jsondata[$i].link+'" class="hx_title">'+jsondata[$i].name+'&nbsp;['+jsondata[$i].num+']<span class="typename">'+jsondata[$i].type+'</span></a></dd>';
                }
            }else{
...
/*             $('.mac_history_box').css({'left':position.left,'top':(position.top+h)}); */
            ...
            if($(".mac_history_set").attr('data-name')){
                ...
                MAC.History.Set($that.attr('data-name'),$that.attr('data-link'),$that.attr('data-type'),$that.attr('data-num'),$that.attr('data-pic'));
            }
        },
        'Set':function(name,link,type,num,pic){
            ...
            if(jsondata != undefined){
                ...
                for($i=0;$i<this.Json.length;$i++){
                    if(this.Json[$i].name == name){
                        ...
                        //this.Json.splice($i,1);
						//alert(this.Json[$i].name);
						
						//return false;
                    }
                }
                jsonstr = '{log:[{"name":"'+name+'","link":"'+link+'","type":"'+type+'","num":"'+num+'","pic":"'+pic+'"},'
				
                for($i=0; $i<(this.Limit-1); $i++){
					
                    if(this.Json[$i]){
                        jsonstr += '{"name":"'+this.Json[$i].name+'","link":"'+this.Json[$i].link+'","type":"'+this.Json[$i].type+'","num":"'+this.Json[$i].num+'","pic":"'+this.Json[$i].pic+'"},';
                    }else{
                        break;
                    }
                }
